...
function App() {
  ...
  const [prevGivers, setPrevGivers] = useState(
    data.joys.sort((a, b) => (a.date > b.date ? -1 : 1)),
  );
  ...
  return (
    <>
      ...
      <main>
        <div className='previous'>
          ...
          <div className='names-container'>
            {prevGivers.map((giver) => {
              return <Name key={giver.id} setViewDate={setViewDate} date={giver.date} name={giver.name}>
        ...
        <div className='joy'>
          <h2 className='mb-1'>{viewDate === currentDate ? 'Todays Joy' : `Joy from ${viewDate}`}</h2>
          <p className='mb-1'>
            Brought to you by{' '}
            <span>{todaysGiver ? todaysGiver.name : 'Someone'}</span>
          </p>
          <Video giver={todaysGiver} embedId={todaysGiver ? todaysGiver.youtube.id : ''} />
